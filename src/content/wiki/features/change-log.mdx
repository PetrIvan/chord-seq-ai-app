...
Audio export, a major platform upgrade, and automated testing.

- Added audio export: chord progressions can now be [exported](/wiki/features/transpose-import-export#export) as WAV (lossless 16-bit PCM) or MP3, in addition to `.chseq` and MIDI. The audio is rendered in the browser using the same instrument and effects as live playback.
- Upgraded the app to Next.js 16 and React 19, and switched the build to Turbopack.
- Added an automated test suite (unit, integration, and end-to-end) that runs in CI to catch regressions before release.
- Fixed a service worker issue that could cause the app to reload repeatedly.
- Restored client-side navigation for internal wiki links, so moving between wiki pages no longer triggers a full page reload.
- Various dependency updates and maintenance fixes.
...
- Fixed an invalid "contact us" link.
